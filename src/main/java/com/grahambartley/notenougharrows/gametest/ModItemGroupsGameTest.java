...
import com.grahambartley.notenougharrows.ModItemGroups;
...
public final class ModItemGroupsGameTest implements FabricGameTest {
  ...
  public void arrowsItemGroupResolvesByIdentifier(TestContext context) {
    context.assertTrue(
        ...
        "The Not Enough Arrows item group should resolve by " + ModItemGroups.ARROWS_ID);
    context.complete();
  }
  ...
  public void arrowsItemGroupIconIsNotEmpty(TestContext context) {
    context.assertFalse(
        ModItemGroups.icon().isEmpty(),
        "The Not Enough Arrows item group icon should never be empty");
    context.complete();
  }
  ...
  public void everyRegisteredArrowIsOfferedByTheArrowsItemGroup(TestContext context) {
    ...
    for (final RegisteredArrow<?> arrow : ModArrows.registered()) {
      context.assertTrue(
          ...
          "Arrow "
              + arrow.id()
              + " should be offered by the Not Enough Arrows creative tab, which offers "
              + offered);
    }
    context.complete();
  }
  ...
  public void theArrowsItemGroupOffersNothingElse(TestContext context) {
    ...
    context.assertEquals(
        offered.size(),
        registered.size(),
        "The Not Enough Arrows creative tab should offer exactly the mod's arrows, but offers "
            + offered);
    context.complete();
  }
  ...
  private static List<Identifier> displayedItemIds(final TestContext context) {
    ...
    context.assertTrue(group != null, "The Not Enough Arrows item group should be registered");
