...
public final class MoreArrowsCommandGameTest implements FabricGameTest {
  private static final String BATCH = "server-config";
  private static final String SET_MAX_RANGE = "morearrows config grapple maxrangeblocks ";
  ...
  public void aNonOperatorCanStillReadStatus(TestContext context) {
    context.assertTrue(
        run(context, operator(context).withLevel(0), "morearrows status"),
  ...
  public void resettingRestoresTheDefaults(TestContext context) {
    ...
    context.assertTrue(
        run(context, operator(context), "morearrows config reset"),
  ...
  public void excludingABlockFromGravityArrowsPersistsIntoTheWorldSave(TestContext context) {
    ...
    context.assertTrue(
        run(
            context,
            operator(context),
            "morearrows config physics gravityblockexclusions add minecraft:bedrock"),
