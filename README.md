...
A charged crossbow shows it everywhere it is drawn: held, in an inventory or hotbar slot, dropped on the ground, and hanging in an item frame. What it is loaded with rides on the crossbow itself, so every one of those surfaces can answer the question. A drawn bow shows it only while it is held, because the arrow it will fire is found by asking the player holding it, and a slot, the ground, and an item frame have nobody to ask.
