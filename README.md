...
Every in-flight head stands three rows tall at the shoulder and tapers to a single lit pixel at the point, which is the proportion vanilla's own arrow uses. A head drawn flat along the centre line has no depth to lose and reads as a smear rather than a blade once the profile is mirrored and moving, which is the one mistake this canvas punishes hardest.
