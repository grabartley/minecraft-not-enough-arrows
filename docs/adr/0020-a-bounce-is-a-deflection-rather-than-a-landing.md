...
The base layer already offers that shape. An arrow's block-hit hook returns whether vanilla's resolution should run, and returning "keep the arrow, skip the resolution" leaves the projectile mid-flight with its velocity intact. What it also skips, because vanilla's resolution is where it lives, is the call that tells the struck block it was hit by a projectile. That call is what presses a wooden button, rings a bell, and scores a target block.

Skipping it silently would make this the one arrow in the mod that cannot trigger a target block, and a trick-shot arrow that cannot hit a target is a strange thing to ship.
...
Before it deflects, the arrow tells the struck block it was hit, which is the one piece of vanilla's resolution it takes by hand. Buttons, bells and target blocks therefore respond to every bounce rather than only to the shot that finally embeds.
