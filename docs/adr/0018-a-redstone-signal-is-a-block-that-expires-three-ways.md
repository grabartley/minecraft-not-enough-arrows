...
Removal only clears positions that still hold a charge block, so a player who builds over one keeps their block. A scheduled tick also asks the tracker how much longer the charge standing at that position has, and re-books itself for that remainder instead of removing anything. Without that question a charge destroyed early, by a piston or by a player building over it, would leave its tick queued, and a second arrow charging the same position before that tick fired would have its signal cut short at the first charge's expiry. Re-booking rather than simply declining to remove matters because vanilla keys queued ticks on block and position alone and ignores the tick they are due: the second arrow's own scheduled tick is silently dropped as a duplicate, so if the stale tick bowed out without replacing itself, the charge would be left with no scheduled expiry at all.
