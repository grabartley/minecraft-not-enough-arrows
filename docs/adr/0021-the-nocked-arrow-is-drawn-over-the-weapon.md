...
The overlay is drawn in the weapon's own model space, after the weapon's display transformation and before the half-block shift every item model gets. That is what makes one set of offsets correct everywhere: first person and third person, left hand and right hand, and the inventory, all differ only in the display transformation the overlay inherits from the weapon.
