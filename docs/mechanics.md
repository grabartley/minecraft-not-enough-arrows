...
| Taunt | A note block | Draws hostiles already fighting something, within `control.targeting.tauntRadius`, onto whatever it struck for `control.targeting.tauntDurationTicks` |
...
| Allegiance | A golden apple | The hostile it hits fights for the shooter for `control.allegiance.durationTicks`, then is handed back |
...
| Allegiance and whose side it is on | The turned mob stops fighting the shooter and goes after whoever is attacking the shooter inside `control.allegiance.defendRadius`, rechecked each tick so it follows the fight. When the duration runs out its target is released and it is an ordinary hostile again. It turns only a hostile mob, never a player, and a dispensed arrow has nobody to defend |
...
| Disarm and the item | The item is thrown `control.disarm.throwDistance` blocks in the direction the target is facing, so the target has to go and fetch it rather than picking it straight back up. It is never destroyed, never moved to the shooter, and a worn armour piece is never taken. A distance of zero drops it at their feet |
| Disarm and getting it back | Vanilla lets most mobs pick nothing up, so a disarmed skeleton would never recover its bow and the arrow would read as a permanent theft. A mob this arrow disarms is allowed to pick items up from then on, which is what makes the effect a setback rather than a removal |
